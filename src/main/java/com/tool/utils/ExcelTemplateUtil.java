...
public class ExcelTemplateUtil {
     ...
     * 通过模板Excel的路径初始化
     */
    public ExcelTemplateUtil(String path) {
        this.path = path;
        init();
    }
                    ...
                    if (c != null) {
                        if (c.getCellType() == CellType.STRING) {
                            if ("${}".equals(c.getStringCellValue())) {
                                ...
                                String value = null;
                                try {
                                    value = valueList.stream().findFirst().orElse(null);
                                } catch (Exception e) {
                                    //e.printStackTrace();
                                }
                                c.setCellValue(value);
...
//                }
    ...
    public boolean equals(Object o) {
        ...
        if (o == this)
            return true;
        if (!(o instanceof ExcelTemplateUtil))
            return false;
        if (examine() ^ ((ExcelTemplateUtil) o).examine())
            return false;
        return Objects.equals(path, ((ExcelTemplateUtil) o).path);
    }
